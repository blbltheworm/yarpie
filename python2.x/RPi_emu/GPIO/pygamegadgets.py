...
class ListGadget(pygameGadget):
    ...
    def edit_line(self, id, text = None, fontcolor = None, color = None):
        """
            Modify an existing line, identified by its id
        """
    
        if text:
            self.lines[id].text = text
        
        if fontcolor:
            self.lines[id].fontcolor = fontcolor
        
        if color:
            self.lines[id].fontcolor = color
...
class PanelGadget(pygameGadget):
    ...
    def selectpanel(self, id):
        """
        id: id of the panel currently selected
        """
        ...
        for gadget in self._panels[id].gadgets:
            gadget.hide = False
...
class RadioGadgetGroup(object):
    ...
    def __init__(self, radiogadgets):
        ...
        self.id = 0 #ID of the Element in gadgetlist which is currently active
    ...
    def check_events(self, events):
        ...
        if counter < len(self._gadgetlist):
            ...
            self.id = counter #store the position of the currently active item in the list
