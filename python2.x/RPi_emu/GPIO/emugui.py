...
class event_detect(object):
    ...
    """
    def __init__(self, cid, bouncetime, edge, func):
        ...
        self.cid         = cid         #ID of the GP-Array
...
class emugui(threading.Thread):
    ...
    def __init__(self, xoffset = 150, yoffset = 20):
        ...
        self.curnumsys = [16, 16] #stores the number system currently used in the i2c-panel (index=0), i2c-autodetect (1) panel
        #curnumsys is stored to be able to convert the current deviceid/data/register if changing the numersystem via the radio buttons 
    ...
    def _check_events(self): 
        ...
        for event in self.events:           
            if tmpedge[event.cid] != 0:
                if time.time() > event.silence:
                    if (event.edge == BOTH) or (event.edge == RISING and tmpedge[event.cid] == 1) or (event.edge == FALLING and tmpedge[event.cid] == -1):
                        ...
                        if not event.func is None:
                            if self.boardmode == BCM:
                                event.func(event.cid+2)
                            elif self.boardmoe == BOARD:
                                event.func(self.boardmap[event.cid+2])

    def add_event(self, cid, edge, bouncetime, callback):
        """
        ...
        """
        self.events.append(event_detect(cid, bouncetime, edge, callback))
    
    def remove_event(self, cid):
        """
